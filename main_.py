...
for net in nets:
    ...
    for call in range(SIM_NUM_CALLS):
        ...
        for a in algs:
            (t,route,wavelength,groomed) = a.rwa(net[i%6], o, d, holding_time,cr)
            i+=1
    ...
    for a in algs:
        ...
        result.append({'Network' : net[0].name, 'Algorithm' : a.name, 'SIM_NUM_CALLS' : SIM_NUM_CALLS, 'Block_count' : sum_block_count, 'Blocking_Probability' : blocking_probability, 'Traffic' : traffic})

rs = pd.DataFrame(columns=['Network', 'Algorithm', 'SIM_NUM_CALLS','Block_count', 'Blocking_Probability', 'Traffic'])
for i in range(len(result)):
    rs.append(result[i], ignore_index=True)
rs.to_csv('Result_calls150.csv')
print('Done')
